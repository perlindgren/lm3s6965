#[doc = "Reader of register VALUE"]
pub type R = crate::R<u32, super::VALUE>;
#[doc = "Writer for register VALUE"]
pub type W = crate::W<u32, super::VALUE>;
#[doc = "Register VALUE `reset()`'s with value 0"]
impl crate::ResetValue for super::VALUE {
    type Type = u32;
    #[inline(always)]
    fn reset_value() -> Self::Type {
        0
    }
}
#[doc = "Reader of field `WDT_VALUE`"]
pub type WDT_VALUE_R = crate::R<u32, u32>;
#[doc = "Write proxy for field `WDT_VALUE`"]
pub struct WDT_VALUE_W<'a> {
    w: &'a mut W,
}
impl<'a> WDT_VALUE_W<'a> {
    #[doc = r"Writes raw bits to the field"]
    #[inline(always)]
    pub unsafe fn bits(self, value: u32) -> &'a mut W {
        self.w.bits = (self.w.bits & !0xffff_ffff) | ((value as u32) & 0xffff_ffff);
        self.w
    }
}
impl R {
    #[doc = "Bits 0:31 - Watchdog Value"]
    #[inline(always)]
    pub fn wdt_value(&self) -> WDT_VALUE_R {
        WDT_VALUE_R::new((self.bits & 0xffff_ffff) as u32)
    }
}
impl W {
    #[doc = "Bits 0:31 - Watchdog Value"]
    #[inline(always)]
    pub fn wdt_value(&mut self) -> WDT_VALUE_W {
        WDT_VALUE_W { w: self }
    }
}
